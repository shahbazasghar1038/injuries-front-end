  ...
  const handleongoingMedicalProvider = (type) => {
    ...
    medicalRecordRequest(model, provider?.id)
      .then((response) => {
        ...
        message.success(
          response?.message || "Record request sent successfully"
        );
      })
      .catch((err) => {
          ...
          <div className="flex items-start gap-3 justify-between w-full relative flex-[0_0_auto]">
            ...
            <div className="cursor-pointer inline-flex p-2 pr-0 items-center justify-center gap-2 rounded-lg overflow-hidden shadow-shadow-xs">
              <div
                ...
                onClick={() => EditProviderDetailshowModal()}  >
                <img
...
{!isDoctor && <>
          <div className="flex items-start gap-6 self-stretch w-full relative flex-[0_0_auto]">
            ...
            <button
              disabled={provider?.BillRequest === "Requested"}
              onClick={() => handleongoingMedicalProvider("bills")}
              className={`all-[unset] ${
                provider?.BillRequest == "Requested"
                  ? "bg-[#F79009] text-[#fff]"
                  : "bg-[#ECF3FF] text-[#3641F5]"
              }  box-border flex items-center justify-center gap-2 px-4 py-3 relative flex-1 grow rounded-lg overflow-hidden border border-solid border-[#e4e7ec]`}
            >
              <div className="mt-[-1.00px] relative w-fit whitespace-nowrap fs-14 fw-500">
                {provider?.BillRequest == "Requested"
                  ? "Bills Requested"
      ...
      <SubmissionModal open={isModalOpen} onClose={handleModalClose}>
        <ProviderEditDetail provider={provider} onSuccess={handleFetchSignleCase} onClose={handleModalClose} />
      </SubmissionModal>
    </div>
  );
};
