...
import ProviderCardsLien from "./partials/ProviderCardsLien";
...
const handleFetchSignleCase = () => {
  ...
  getSingleCase2(id)
    .then((response) => {
      ...
      console.log("Lien single case data :", response);
      setLoading(false);
    })
    .catch((err) => {
      console.error(err);
              ...
              {filteredQuestions.map((item) => (
                ...
                >
                  ...
                  <div
                    ...
                    onClick={() => window.open(item.files, "_blank")}
                  >
                    <div className="relative w-fit status-tag">
                      {item.files ? <Icons.downloadIcon /> : ""}
                    </div>
                  </div>
...
export default CaseDetailPageLien;
